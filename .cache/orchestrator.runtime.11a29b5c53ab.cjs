...
exports.runReportAgent = exports.runReallocationAgent = exports.runMatchingAgent = exports.runGapDetectionAgent = exports.runScoringAgent = exports.runExtractionAgent = exports.runIngestionAgent = exports.runOrchestrator = void 0;
...
const normalizeSkillTag = (value) => (value || '')
    .toLowerCase()
    .trim()
    .replace(/\s+/g, '_')
    .replace(/[^a-z0-9_]/g, '');
const DETERMINISTIC_FALLBACK_CASES = [
    {
...
const isMeaningfulIssueSummary = (summary) => {
    ...
    if (positiveOrSummaryPatterns.some((pattern) => pattern.test(lower)))
        return false;
    return issueSignals.test(lower);
};
const normalizeIssueTemplate = (issue) => ({
...
const getDeterministicFallbackCase = (rawInput) => {
...
const buildGuaranteedFallbackIssues = (rawInput) => {
...
const extractAffectedCountFromText = (text) => {
...
const extractFallbackIssuesFromText = (rawInput) => {
    ...
    for (const candidate of candidates) {
        ...
        if (summary.length < 15)
            continue;
        if (!isMeaningfulIssueSummary(summary))
            continue;
...
const safeParseGeminiIssues = (content) => {
    ...
    const parseAttempts = [];
    if (normalized)
        parseAttempts.push(normalized);
    const arrayMatch = normalized.match(/\[[\s\S]*\]/);
    if (arrayMatch?.[0])
        parseAttempts.push(arrayMatch[0]);
    const objectMatch = normalized.match(/\{[\s\S]*\}/);
    if (objectMatch?.[0])
        parseAttempts.push(objectMatch[0]);
...
const normalizeExtractedIssues = (issues) => {
    const mapped = (issues || []).map((issue) => ({
        ...
        location: issue.location ?? null,
        affected_count: typeof issue.affected_count === 'number'
...
const extractWithGemini = async (rawInput, apiKey) => {
...
${safeRawInput}`;
    try {
        ...
        const parsed = safeParseGeminiIssues(content);
...
const assignWithGemini = async (issues, volunteers, apiKey) => {
...
const generateActionPlanSummaryWithGemini = async (state, apiKey) => {
    const planContext = {
...
const scoreWithGemini = async (issues, apiKey) => {
...
Issues:
${JSON.stringify(issues.map((i, idx) => ({
        issue_index: idx,
...
const detectSkillsWithGemini = async (issues, apiKey) => {
...
${JSON.stringify(issues.map((i, idx) => ({
    ...
    return parsed
        ...
        required_skills: Array.isArray(item.required_skills)
            ? item.required_skills.map((s) => normalizeSkillTag(s)).filter(Boolean)
            : [],
    }));
};
// ============ TOOL: INGEST ============
const ingestTool = async (state) => {
...
const gapTool = async (state) => {
    ...
    if (enrichedIssues.some((i) => !Array.isArray(i.required_skills) || i.required_skills.length === 0)) {
        enrichedIssues = enrichedIssues.map((issue) => ({
            ...issue,
            required_skills: Array.isArray(issue.required_skills) && issue.required_skills.length > 0
    ...
    for (const issue of enrichedIssues) {
        const requiredSkills = Array.isArray(issue.required_skills) ? issue.required_skills : [];
...
const findBackupVolunteer = (assignedVolunteerId, issue, volunteers, volunteerLoads) => {
    const issueSkills = Array.isArray(issue.required_skills) && issue.required_skills.length > 0
        ? issue.required_skills
        : [normalizeSkillTag(issue.sector)].filter(Boolean);
    const activeVolunteers = volunteers.filter((v) => {
        if (!v.is_active || v.id === assignedVolunteerId)
            return false;
        const volunteerSkills = (v.skills || []).map((s) => normalizeSkillTag(s));
    ...
    return activeVolunteers.reduce((prev, curr) => (volunteerLoads[prev.id] || 0) <= (volunteerLoads[curr.id] || 0) ? prev : curr);
};
// ============ TOOL: MATCH ============
const matchTool = async (state) => {
    ...
    for (const { issue, idx } of sortedIssues) {
        ...
        const requiredSkills = Array.isArray(issue.required_skills) && issue.required_skills.length > 0
            ? issue.required_skills
            : [normalizeSkillTag(issue.sector)].filter(Boolean);
        ...
        if (candidates.length > 0) {
            // Pick least-loaded
            const best = candidates.reduce((prev, curr) => (volunteerLoads[prev.id] || 0) <= (volunteerLoads[curr.id] || 0) ? prev : curr);
            // Find backup volunteer (different from primary)
...
// ============ TOOL: REALLOCATE ============
const reallocateTool = async (state) => {
    ...
    const overloaded = state.volunteers.filter((v) => v.is_active && volunteerLoads[v.id] > (v.availability_hours_per_week || 10) / 2);
    for (const overvol of overloaded) {
        ...
        for (const { i: issue, idx: issueIdx } of theirIssues.slice(0, Math.ceil(theirIssues.length / 2))) {
            ...
            const candidates = state.volunteers.filter((v) => v.id !== overvol.id &&
                v.is_active &&
                (v.skills?.includes(issue.sector) || true) &&
                volunteerLoads[v.id] < (v.availability_hours_per_week || 10) / 2);
            if (candidates.length > 0) {
                const best = candidates.reduce((prev, curr) => volunteerLoads[prev.id] <= volunteerLoads[curr.id] ? prev : curr);
                if (issueIdx >= 0) {
...
const think = (state) => {
    if (state.currentStep === 'starting' || state.currentStep === 'ingestion')
        return 'extract';
    if (state.currentStep === 'extraction')
        return 'score';
    if (state.currentStep === 'scoring')
        return 'gap';
    if (state.currentStep === 'gap_detection')
        return 'match';
    if (state.currentStep === 'matching')
        return 'reallocate';
    if (state.currentStep === 'reallocation')
        return 'report';
    // Fallback safety
...
// ============ MAIN ORCHESTRATOR ============
const runOrchestrator = async (rawInput, volunteers, onStepComplete) => {
        ...
        state = {
            ...
            agentLogs: [
                ...result.state.agentLogs,
                {
                    ...
                    timestamp: new Date().toISOString(),
                    decision: `Executed ${nextTool} tool (confidence: ${(result.confidence * 100).toFixed(0)}%) — ${nextTool === 'extract'
